...
Problem 3 delves into the idea of effect size and whether a false null
hypothesis will be rejected. The probability that a false null
hypothesis is rejected is referred to as power, and depends on several
factors including: the sample size, effect size, and the error variance.

## Set the Regression Design Elements

In order to conduct a simulation to explore power in a simple linear
regression, certain design elements were set: n=30, xi1 draws from a
standard Normal distribution, β0=2, and σ2=50.
...
variables for future analysis includes the “beta1” variable (true),
“estimate” variable (estimate), and the “p.value” variable.
...
increasing relationship between effect size and power. As the true β1
increases, power increases, following a sigmoid population growth curve.
The slope of the curve increases from flat, to steep, and then appears
...
plot comparing the average estimate and true value of beta1, filtered
among samples for which the null was rejcted.
...
the true value of β1, which the greatest divergence when the true value
of β1 is 1.
...
p-values based on our regression simulation, these values are those that
